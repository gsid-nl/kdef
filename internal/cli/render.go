...
func runRender(cmd *cobra.Command, args []string) error {
	...
	if len(config.Deployments) == 0 && len(config.CronJobs) == 0 && len(config.ConfigMaps) == 0 && len(config.Secrets) == 0 && len(config.SealedSecrets) == 0 && len(config.PersistentVolumeClaims) == 0 {
